...
class CSVStrategy(Strategy):
    def __init__(self, sample_input):
        try:
            ...
            sample_input.seek(0)
            self.delimiter = csv.Sniffer().sniff(sample_input.read(1024)).delimiter
            sample_input.seek(0)
            self.csv = [row for row in csv.reader(sample_input, delimiter=self.delimiter)]
    ...
    def generate_input(self):
        ...
        yield self.remove_delimiters()   # invalid csv - remove all self.delimiters
        yield self.change_delimiters()   # change delimiters
        
        # 12 * len(self.csv), 'modify # of lines'
        yield self.lines_csv()   # check number of lines

        # 12 * len(self.csv), 'modify the fields'
        yield self.fields_csv()  # check fields - can return number of expected fields
        
        # 12 * len(self.csv), 'overflowing fields'
        yield self.overflow_fields() # overflowing fields with string
        
        # 12 * len(self.csv), 'format strings'
        yield self.format_string()   # string format
        
        # 12 * len(self.csv), 'modifying headers'
        yield self.change_header()   # change first line

        # 12 * len(self.csv), 'overflowing numbers'
        yield self.overflow_numbers()# overflow intergers

        # 12 * len(self.csv), 'random bit flips'
        for _ in range(0, 20):  # bit flipping
            yield self.byte_flip()
            ...
            for x in range(len(self.csv)):
                ...
                try:
                    ...
                    if x > 0:
                        ...
                        expected_field_no = x
                    break
                error.append(self.delimiter.join(self.csv[x]) + b'\n')
        ...
        for length in range(0, 1000, 100):
            ...
            for l in range(0, length):
                if l < len(self.csv):
                    ...
                    error.append(self.delimiter.join(self.csv[l]) + b'\n')
                else:
                    yield self.delimiter.join(self.csv[len(self.csv) - 1])
                    error.append(self.delimiter.join(self.csv[len(self.csv) - 1]) + b'\n')
